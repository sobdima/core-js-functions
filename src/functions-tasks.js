...
function getPolynom(...arg) {
  ...
  return function res(x) {
    return arg.reduce((acc, item, index) => {
      const power = arg.length - index - 1;
...
function logger(func, logFunc) {
  return function R(...args) {
    ...
    const stringOfArgs = args.map((item) => JSON.stringify(item)).join(',');
    const stringOfLog = `${funcName}(${stringOfArgs})`;

    logFunc(`${stringOfLog} starts`);
    const result = func(...args);
    logFunc(`${stringOfLog} ends`);
...
function partialUsingArguments(fn, ...args1) {
  return function R(...restArgs) {
    return fn(...args1, ...restArgs);
  };
}
